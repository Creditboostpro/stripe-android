...
class IntentConfirmationHandlerTest {
    ...
    fun `On init with 'SavedStateHandle', should receive result through 'awaitIntentResult'`() = runTest {
        val confirmationOption = EXTERNAL_PAYMENT_METHOD

        val savedStateHandle = SavedStateHandle().apply {
            set("AwaitingPaymentResult", true)
            set("PaymentConfirmationArguments", DEFAULT_ARGUMENTS.copy(confirmationOption = confirmationOption))
        }
        ...
        val externalPaymentMethodsCallbackHandler = FakeResultHandler<PaymentResult>()

        intentConfirmationHandler.registerWithCallbacks(
            externalPaymentMethodsCallbackHandler = externalPaymentMethodsCallbackHandler
        )

        externalPaymentMethodsCallbackHandler.onResult(PaymentResult.Completed)
